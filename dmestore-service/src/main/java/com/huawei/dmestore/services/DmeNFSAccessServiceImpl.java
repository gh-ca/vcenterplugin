...
public class DmeNFSAccessServiceImpl implements DmeNFSAccessService {
                            ...
                            if (!StringUtils.isEmpty(vmwareObjectId)) {
                                ...
                                if (dvrMap != null && dvrMap.get(vmwareObjectId) != null) {
                                    ...
                                    double reserveCapacity =
                                        (ToolUtils.getDouble(jo.get("capacity")) - ToolUtils.getDouble(
                                            jo.get("freeSpace"))) / ToolUtils.GI;
                                    ...
                                    String fsUrl = "";
                                    try {
                                        fsUrl = DmeConstants.DME_NFS_FILESERVICE_DETAIL_URL.replace("{file_system_id}",
                                            dvr.getFsId());
                                        ResponseEntity<String> responseTuning = dmeAccessService.access(fsUrl,
                                            HttpMethod.GET, null);
                                        if (responseTuning.getStatusCodeValue() / 100 == 2) {
                                            JsonObject fsDetail = gson.fromJson(responseTuning.getBody(),
                                                JsonObject.class);
                    ...
                    if (dataJson != null) {
                        ...
                        for (String fsId : fsIds) {
                            ...
                            if (statisticObject != null) {
                                ...
                                nfsDataInfo.setBandwidth(ToolUtils.jsonToFloat(
    ...
    public List<Map<String, Object>> getHostsMountDataStoreByDsObjectId(String dataStoreObjectId) throws DmeException {
        ...
        LOG.info("getHostsMountDataStoreByDsObjectId===" + (lists == null ? "null" : (lists.size())));
        return lists;
    }
    ...
    public List<Map<String, Object>> getClusterMountDataStoreByDsObjectId(String dataStoreObjectId)
        ...
        LOG.info("getClustersMountDataStoreByDsObjectId===" + (lists == null ? "null" : (lists.size())));
        return lists;
    }
