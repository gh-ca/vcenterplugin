            ...
            if (!FileUtils.isWindows()) {
                try {
                    ...
                    if (new File(oldDbFile).exists() && !newDbFile.exists()) {
                        // move db file
                        LOGGER.info(
                            "Copying DB file from " + H2DataBaseDao.getDbFileName() + " to " + newDbFile.getName());
                        ...
                        Files.copy(Paths.get(oldFolder + "/" + FileUtils.BASE_FILE_NAME),
                            Paths.get(newFolder + "/" + FileUtils.BASE_FILE_NAME), StandardCopyOption.REPLACE_EXISTING);
                        Files.copy(Paths.get(oldFolder + "/" + FileUtils.WORK_FILE_NAME),
                            Paths.get(newFolder + "/" + FileUtils.WORK_FILE_NAME), StandardCopyOption.REPLACE_EXISTING);
