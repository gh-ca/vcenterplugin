...
public class HttpRequestUtil {
  ...
  public static <T> ResponseEntity<T> requestWithBody(String url, HttpMethod method,
    ...
    if (responseEntity == null) {
      throw new DMEException(CODE_ESIGHT_CONNECT_EXCEPTION, "Esight not found error");
    }

    if (responseEntity.getStatusCode().value() >= 400
        && responseEntity.getStatusCode().value() <= 600) {
      throw new DMEException(responseEntity.getStatusCode().name(), "Esight error");
    }
  ...
  private static String encode(String str) {
    try {
      ...
      throw new DMEException("Esight URL Encode error");
    }
  }
