...
import { transDateFormat } from 'app/app.helpers';
...
export class VmfsPerformanceService {
  ...
  getIopsChart(
    ...
    return new Promise((resolve, reject) => {
      this.http
        .post('datastorestatistichistrory/vmfs', {
            ...
            if (response.code === '200' && response.data !== null) {
              ...
              uppers.forEach(item => {
                ...
                for (const key of Object.keys(pavgData)) {
                  this.iopsChart.series[0].data.push({
                    value: Number(pavgData[key]),
                    symbol: 'none',
                  });
                }
              });
              ...
              lower.forEach(item => {
                ...
                for (const key of Object.keys(lavgData)) {
                  this.iopsChart.series[1].data.push({
                    value: Number(lavgData[key]),
                    symbol: 'none',
                  });
                }
              });
