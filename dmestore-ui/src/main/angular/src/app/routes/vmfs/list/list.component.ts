...
export class VmfsListComponent implements OnInit {
  ...
  refreshVmfs() {
    ...
    debugger;
    // 进行数据加载
    const dataResHandler = (result: any) => {
    ...
    /* TODO:  仅测试用 */
    // dataResHandler(mockData[URLS_LIST_SERVICE.ACCESSVMFS_LISTVMFS]);
    this.remoteSrv.getData().subscribe(dataResHandler);
  }
    ...
    if (this.wwns.length > 0) {
      ...
      const chartDataResHandler = (chartResult: any) => {
        if (chartResult.code === '200' && chartResult.data != null) {
          ...
          const listMap = chartResult.data.reduce((_listMap, item) => {
            _listMap[item.wwn] = item;
            return _listMap
          }, {});
          ...
          this.list = this.list.map((i) => {
            ...
            const {
              ...
              readResponseTime,
              writeResponseTime,
            } = listMap[wwn] || { maxIops: null, minIops: null, iops: null, bandwidth: null, latency: null, maxBandwidth: null, minBandwidth: null, readResponseTime: null, writeResponseTime: null, };
            return {
              ...i,
              iops,
              maxIops,
